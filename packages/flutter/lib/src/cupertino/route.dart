...
mixin CupertinoRouteTransitionMixin<T> on PageRoute<T> {
  ...
  bool canTransitionTo(TransitionRoute<dynamic> nextRoute) {
    ...
    return nextRouteIsNotFullscreen && ((nextRoute is CupertinoRouteTransitionMixin) || nextRouteHasDelegatedTransition);
  }
...
class _PageBasedCupertinoPageRoute<T> extends PageRoute<T> with CupertinoRouteTransitionMixin<T> {
  _PageBasedCupertinoPageRoute({required CupertinoPage<T> page, super.allowSnapshotting = true}) : super(settings: page) {
    assert(opaque);
  }
...
class _CupertinoEdgeShadowPainter extends BoxPainter {
  _CupertinoEdgeShadowPainter(this._decoration, super.onChanged) : assert(_decoration._colors == null || _decoration._colors.length > 1);
