...
void main() {
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      FadeTransition widget2Opacity = tester.element(find.text('Page 2')).findAncestorWidgetOfExactType<FadeTransition>()!;
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      Offset widget2TopLeft = tester.getTopLeft(find.text('Page 2'));
      final RenderDecoratedBox box = tester.element(find.byKey(page2Key)).findAncestorRenderObjectOfType<RenderDecoratedBox>()!;
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      bool isTransitioningWithoutSnapshotting(Finder of) {
        ...
        return layers.whereType<OpacityLayer>().length == 1 && layers.whereType<TransformLayer>().length == 1;
      }

      bool isSnapshotted(Finder of) {
        final Iterable<Layer> layers = findLayers(of);
        // The scrim and the snapshot image are the only two layers.
        return layers.length == 2 && layers.whereType<OffsetLayer>().length == 1 && layers.whereType<PictureLayer>().length == 1;
      }
